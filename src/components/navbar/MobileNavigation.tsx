    ...
    <AnimatePresence>
      {state && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -8 }}
          className="fixed z-[10000] w-screen backdrop-blur-3xl md:hidden top-[73px] bottom-0 px-6 flex flex-col items-center justify-center gap-12"
        >
          {/* Nav links */}
          ...
          >
            {socials.map(({ href, label, Icon }) => (
              <Link
                ...
                aria-label={label}
                onClick={() => setState(false)}
                className="border-2 border-foreground/20 rounded-full p-2 text-muted-foreground hover:border-primary/50 hover:text-foreground transition-all"
              >
